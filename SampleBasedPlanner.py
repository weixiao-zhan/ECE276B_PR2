...
class SampleBasedPlanner(ABC):
    ...
    def build_path(self, save_path = None):
        path = nx.shortest_path(self.graph, source=self.start, target=self.goal, weight='weight')
...
class RRT(SampleBasedPlanner):
    ...
    def sample(self, m = 50, first_path_exit = True):
        self.graph.add_node(self.start)
        for i in tqdm(range(self.n)):
            rand_node, step = (self.env.sample_free(), self.radius/2) if i % m != 0 else (self.goal, float('inf'))
            ...
            if not self.env.line_collide(nn_node, new_node):
                ...
                if first_path_exit and self.goal in self.graph.nodes:
                    print("first path exit")
                    return
...
class RRTstar(SampleBasedPlanner):
    ...
    def sample(self, m = 50, step=0.1, plot=True):
        self.graph.add_node(self.start)
        for i in tqdm(range(self.n)):
            if i % m == 0 and plot:
                self.plot_graph()
            ...
            if min_cost_node is not None:
                self.graph.add_edge(min_cost_node, new_node)
